...
int count = 0;
    ...
    for(int o = 0; o < t; o++){
            R[l][o] = (((a*ciphertext[o]) + (l+1)) & 0x0000000000FFFFFF); //Generating a key by using chaining function of the ciphertext
            for(int r =0; r<t; r++){
                    for(int j = 0; j < m; j++){
            ...
            for(int i = 0; i < t; i++){
                ...
                mkey[i+1] = (((a*z[i]) + (l+1)) & 0x0000000000FFFFFF); //generating all keys in the chain
            }
        }
}
        }
