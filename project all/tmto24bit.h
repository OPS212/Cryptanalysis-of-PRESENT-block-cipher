#ifndef TMTO24BIT_H_INCLUDED
#define TMTO24BIT_H_INCLUDED
...
uint64_t printendkey(uint64_t x, uint64_t k, int b)
{
    ...
    ks[i+1] = (((a*y[i]) + b) & 0x0000000000FFFFFF);
    //printf("Next key is: %llx\n", ks[i+1]); //
}
printf("Last key is: %llx\n", ks[255]);
return ks[255];
}
